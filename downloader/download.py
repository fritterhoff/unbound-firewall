...
class UnboundDownloader(object):
    ...
    def prepareDatasets(self):
        ...
        source, self.categories, self.ip_range_lists = self._read_providers(
            os.path.join(self.__location__, "providers.json"))
        ...
        with open(os.path.join(self.__location__,"./zone_template")) as file:
            self.header = file.read().replace("{SERIAL}", datetime.datetime.now().strftime(
                "%Y%m%d%H")).replace("{HOSTNAME}", socket.getfqdn())
    ...
    def domain_data(self):
        for category_combination in range(len(self.categories)):
            ...
            domains = "\n".join(
                self.category_domains[str(category_combination)]) + "\n"
            ...
            if os.path.exists(file_path):
                with open(file_path, "r") as file:
                    ...
                    if present == domains:
                        print(
                            f'Content of type {description} is unchanged --> Leaving file unchanged')
                        continue
    ...
    def ip_data(self):
        ...
        with open(self.unbound_conf_dir + "rpz.ip-range.zone", "w+") as file:
            ...
            for ip in ips:
                file.write(
                    f'{ip:{maxLength}} CNAME {self.ip_range_lists["target"]}\n')
